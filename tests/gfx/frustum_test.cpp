...
TEST_CASE("Frustum::is_inside()") {
    ...
    SECTION("checks if an oriented bounding box is inside in the frustum") {
        static constexpr std::array<glm::vec3, 8> frustum_points {
            // Near plane
            glm::vec4(-1.0f, 1.0f, 0.1f, 1.0f), // Near bottom left
            glm::vec4(1.0f, 1.0f, 0.1f, 1.0f),  // Near bottom right
            glm::vec4(-1.0f, -1.0f, 0.1f, 1.0f),  // Near top left
            glm::vec4(1.0f, -1.0f, 0.1f, 1.0f),   // Near top right
            // Far plane
        ...
        const Frustum frustum(frustum_points);
        const OrientedBoundingBox3D obb(
            glm::mat3(
